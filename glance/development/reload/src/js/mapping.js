...
function setCurrentPosition(pos, $http, glancepub, glancesms) {
    ...
    publish("my_channel", pos.coords.latitude, pos.coords.longitude, $http, glancepub);
    ...
    google.maps.event.addDomListener(shareControlNewUI, 'click', function () {
        forge.contact.select(function(contact) {
            ...
            forge.logging.log('Sharing to '+'+91'+contact.phoneNumbers[0].value);
...
function watchCurrentPosition($http, glancepub) {
    ...
    /*google.maps.event.addDomListener(shareControlUI, 'click', function () {
    forge.contact.select(function(contact) {
        forge.logging.log('shareMyLoc bef - ' + shareMyLoc);
        shareMyLoc = !shareMyLoc;
        forge.logging.log('shareMyLoc aft - ' + shareMyLoc);
        if(shareMyLoc == true){
        forge.sms.send({
            body: "http://geobus.s3-website-us-west-2.amazonaws.com/",
            to: contact.phoneNumbers[0].value
        }, function () {
            forge.logging.log('name - ' + contact.phoneNumbers[0].value);
            forge.logging.log('phone - ' + contact.name.value);
            postMessage();
        });
        }
    }, function() {
        //postMessage();
        forge.logging.log('No contact selected!');
...
function setMarkerPosition(position, $http, glancepub) {
    ...
    if (shareMyLoc) {// && speed > 0 
        forge.logging.log("Speed : " + (speed < 0 ? 0 : speed));
        publish("my_channel", position.coords.latitude, position.coords.longitude, $http, glancepub);
        //document.getElementById('maphead').setAttribute('view-title').replace("You are currently not sharing", "You are currently sharing");
        //forge.logging.log("maphead HTMLLLLL: "+document.getElementById('maphead').getAttribute('view-title'));
    }
