...
.controller('AppCtrl', function ($scope, $ionicModal, $timeout, Auth, $state, glancemail, $http, glancecreateaccount) {
    $scope.loginData = {};
    $scope.registerData = {};
    ...
    $scope.doLogin = function () {
      ...
      ref.authWithPassword({
        ...
        if (error) {
          ...
          forge.logging.log("Authenticated successfully! -->  "+JSON.stringify(authData));
          $state.go("app.map");
        }
      });
    };
    ...
    $scope.doRegister = function () {
      ...
      var userData = createaccount($scope.registerData.email, $scope.registerData.name, $scope.registerData.phone, $scope.registerData.password, $http, glancecreateaccount)
      if(userData != null && userData.uid != null){
        sendverificationemail($http, glancemail, $scope.registerData.email,userData.uid)
      } else {
        forge.logging.log("Registration failed - "+(userData != null && userData.error != null) ? userData.error : "");
      }
